...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        for _ in range(lower_bound):
            if not p.entrance.is_hive:
                p = p.entrance
    
        step = 0
        while not p.is_hive and step <= upper_bound-lower_bound:
            if p.bees:
        ...
        while p.exit is not None:
            ...
            if p.ant is not None:
                if p.ant.is_container:
                    p.ant.double()
                    if p.ant.ant_contained is not None:
...
def apply(func, bee, length):
    origin = bee.action
    new = func(bee.action, bee)
    
    def change(gamestate):
        nonlocal length
        if length > 0:
            new(gamestate)  
            length -= 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        place = self.place
        distance = 0
        while not place.entrance.is_hive:
