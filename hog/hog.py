...
def boar_brawl(player_score, opponent_score):
    ...
    ply_ones = player_score % 10
    score = 3 * abs(op_tens - ply_ones) if op_tens - ply_ones != 0 else 1
    return score
    # END PROBLEM 2
...
# average_dice = make_averaged(roll_dice, 40)
# print(average_dice(2, dice))
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max = 10
    max_sum = average_dice(max, dice)
    for i in range(9,0,-1):
        if average_dice(i, dice) >= max_sum:
            max = i
            max_sum = average_dice(i, dice)
    return max
    # END PROBLEM 9
