...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 or len(source_words) == 0:
        return 0.0
    x = 0
    k = min(len(typed_words), len(source_words))
    for i in range(k):
        if typed_words[i] == source_words[i]:
            x+=1
    return 100*(x/len(typed_words))
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        ...
        # print(result)
        if  result<= limit and result < min:
            min = result
            word = word_list[i]
    return word     
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]: # Feel free to remove or add additional cases
        ...
        # BEGIN
        "*** YOUR CODE HERE ***"
        cost_add = minimum_mewtations(add,source,limit-1)
        if cost_add == 0:
            return 1
        cost_remove = minimum_mewtations(remove,source,limit-1)
        if cost_remove == 0:
            return 1
        cost_substitute = minimum_mewtations(substitute,source,limit-1)
        if cost_substitute == 0:
            return 1
        return  min(cost_add,cost_remove,cost_substitute)+1
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    percentage = correct/len(source)
    dic = {'id':user_id,'progress':percentage}
    upload(dic)
    return percentage
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    time = [[0]*(len(timestamps_per_player[i])-1) for i in range(len(timestamps_per_player))]
    for i in range(len(timestamps_per_player)):
        for j in range(1,len(timestamps_per_player[i])):
            time[i][j-1] = timestamps_per_player[i][j] - timestamps_per_player[i][j-1]
    return words,time
    # END PROBLEM 9
...
def time_per_word_match(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    thiswords, thistime = time_per_word(words, timestamps_per_player)
    return match(thiswords, thistime)
    # END PROBLEM 10
...
def fastest_words(match_object):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    listt =  [[] for _ in player_indices]
    for i in word_indices:
        fastest = min(player_indices, key=lambda j: get_all_times(match_object)[j][i])
        listt[fastest].append(get_word(match_object, i))
    return listt
    # END PROBLEM 11
